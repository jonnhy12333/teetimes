  ...
  return <div class="course-info-backdrop" role="presentation" onPointerDown={(event) => { if (event.target === event.currentTarget) props.onClose() }}>
    <section class="course-info-modal" role="dialog" aria-modal="true" aria-labelledby="course-info-title">
      <header class="course-info-header" classList={{ 'has-image': Boolean(props.course.headerImageUrl) }} style={props.course.headerImageUrl ? { 'background-image': `linear-gradient(180deg, rgb(8 18 12 / 16%) 0%, rgb(8 18 12 / 82%) 100%), url("${props.course.headerImageUrl}")` } : undefined}>
        <CourseAvatar class="course-info-avatar" name={props.course.name} logoUrl={props.course.logoUrl} />
        <div><h2 id="course-info-title">{props.course.name}</h2><p>{props.course.city}, {props.course.state}</p><CourseHours course={props.course} inline /></div>
...
export default function Dashboard() {
  ...
  const [teeTimeDialogOpen, setTeeTimeDialogOpen] = createSignal(false)
  ...
  const [whatsNewMounted, setWhatsNewMounted] = createSignal(false)
  ...
  const closeTeeTimeDetails = () => setTeeTimeDialogOpen(false)
  ...
  onMount(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === 'Escape') { setCoursePickerOpen(false); setCourseFilterOpen(false); setInfoCourse(null); closeTeeTimeDetails(); setWhatsNewOpen(false); setTimelineFullscreen(false) } }
  ...
  const updateLabel = () => {
    currentMinutes()
    if (loading() || refreshing()) return 'Updating…'
  ...
  return <div class="container"><main class="dashboard search-dashboard">
    <Show when={whatsNewMounted()}><Portal><Dialog.Root open={whatsNewOpen()} onOpenChange={(details) => setWhatsNewOpen(details.open)} onExitComplete={() => setWhatsNewMounted(false)}><Dialog.Backdrop class="whats-new-backdrop" /><Dialog.Positioner class="whats-new-positioner"><Dialog.Content class="whats-new-panel"><header><div><span>TEE TIMES</span><Dialog.Title id="whats-new-title">What’s new</Dialog.Title></div><Dialog.CloseTrigger aria-label="Close what’s new">×</Dialog.CloseTrigger></header><div class="whats-new-list"><For each={appReleases}>{(release, index) => <article><div><time>{release.date}</time><Show when={index() === 0}><span>Latest</span></Show></div><h3>{release.title}</h3><ul><For each={release.changes}>{(change) => <li>{change}</li>}</For></ul></article>}</For></div></Dialog.Content></Dialog.Positioner></Dialog.Root></Portal></Show>
            ...
            <For each={resultCourses()}>{(course) => {
              ...
              return <article class="timeline-course-row" classList={{ 'no-matches': !loadingCourseIds().includes(course.id) && !failedCourseIds().includes(course.id) && timeline().items.length === 0 }}>
                <header class="course-card-header timeline-course-header">
                  <button type="button" class="course-avatar-button" aria-label={`View information about ${course.name}`} title={course.name} onClick={() => setInfoCourse(course)}><CourseAvatar name={course.name} logoUrl={course.logoUrl} /></button>
